        ...
        const choices = [
            {
                ...
                description: hasManualToken ? 'Đã có token (cập nhật)' : 'Thiết lập token mới',
                action: async () => {
                    ...
                    if (token && token.trim()) {
                        ...
                        await config.update('authMethod', 'manual', vscode.ConfigurationTarget.Global)
                        vscode.window.showInformationMessage('✅ Đã lưu GitHub token (manual)')
                        await vscode.commands.executeCommand('tomemiru-release.refreshStatusBar')
            ...
            },
            {
                ...
                description: hasManualToken ? 'Xóa token đã lưu' : 'Không có token để xóa',
                action: async () => {
                    if (!hasManualToken) {
            ...
            },
            {
                ...
                action: () => vscode.commands.executeCommand('workbench.action.openSettings', 'tomemiruRelease'),
            },
            {
                label: '$(key) Chọn Auth Method',
                description: 'VSCode / Manual / Env',
                action: async () => {
                    const currentMethod = vscode.workspace.getConfiguration('tomemiruRelease').get<string>('authMethod') || 'vscode'
                    const method = await vscode.window.showQuickPick(
                        [
                            { label: '$(vscode) VSCode OAuth', value: 'vscode', picked: currentMethod === 'vscode' },
                            { label: '$(key) Manual Token', value: 'manual', picked: currentMethod === 'manual' },
                            { label: '$(file) Env File', value: 'env', picked: currentMethod === 'env' },
                        ],
                    ...
                    if (method) {
                        ...
                        vscode.window.showInformationMessage(`✅ Đã đổi auth method: ${method.label}`)
                    }
                },
            },
            {
                ...
                action: () => vscode.commands.executeCommand('workbench.action.openSettings', 'tomemiruRelease.repoPaths'),
            },
        ]
